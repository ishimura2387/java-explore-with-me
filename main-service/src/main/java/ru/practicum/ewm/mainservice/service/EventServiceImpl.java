...
public class EventServiceImpl implements EventService {
    ...
    public List<EventFullDto> getAllWithParam(RequestEventParam requestEventParam) {
        ...
        */
        ...
        if (requestEventParam.getEventRequester().equals(EventRequester.PUBLIC_REQUESTER)) {
        ...
        return setViews(setNumberOfComments(events));
    }
    ...
    public List<EventFullDto> getAllByInitiator(Long userId, Pageable pageable) {
        ...
        return setViews(setNumberOfComments(eventFullDtos));
    }
    ...
    public EventFullDto get(Long userId, Long eventId, EventRequester eventRequester) {
        ...
        Event event = eventRepository.findById(eventId)
                .orElseThrow(() -> new NotFoundException("Ошибка проверки события на наличие в Storage! " +
                        "Событие не найдено!"));
        if (eventRequester.equals(EventRequester.PUBLIC_REQUESTER) && !event.getState().equals(EventState.PUBLISHED)) {
        ...
        return setViews(setNumberOfComments(events)).get(0);
    }
    ...
    private List<EventFullDto> setNumberOfComments(List<EventFullDto> events) {
        ...
        commentsValue = commentRepository.getNumberOfComments(eventsIds);
        int k = 0;
        for (EventFullDto event : events) {
            event.setComments(commentsValue.get(k));
            k++;
        }
        return events;
    }
