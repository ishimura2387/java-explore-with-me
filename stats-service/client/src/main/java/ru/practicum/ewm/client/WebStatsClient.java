...
@ComponentScan
@Slf4j
public class WebStatsClient implements StatsClient {
    private final WebClient webClient;

    public String url;

    public WebStatsClient(@Value("${stats.url:http://localhost:9090}") String url) {
        this.url = url;
    ...
    public  List<ViewStatsDto> getStats(LocalDateTime startLocalDateTime, LocalDateTime endLocalDateTime, List<String> uris, boolean unique) {
        ...
        List<ViewStatsDto> list = webClient
                .get()
                .uri("/stats?start={start}&end={end}&uris={urisString}&unique={unique}", start, end,
                        builder.deleteCharAt(0).toString(), String.valueOf(unique))
