...
public interface EndpointHitRepository extends JpaRepository<EndpointHit, Long> {
    ...
    List<ViewStats> findUnique(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end, @Param("uris") List<String> uris);

    @Query("select s.app as app, s.uri as uri, s.ip as hits from EndpointHit s where s.uri in :uris " +
            "and s.createTime >= :start and s.createTime <= :end group by s.app, s.uri order by hits desc")
    List<ViewStats> findNotUnique(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end, @Param("uris") List<String> uris);
}
